...
- [x] `TEMPLATES['DIRS']` in project's `settings.py`
- [ ] `TEMPLATES['APP_DIRS']` in project's `settings.py`
- [ ] In the app's `urls.py`

If we want to add a custom template directory (outside the app structure), weshould use TEMPLATES['DIRS'] in the project's settings.py.
If we want Django to automatically search inside the app’s templates directories, we should enable TEMPLATES['APP_DIRS'] by setting it to True.
